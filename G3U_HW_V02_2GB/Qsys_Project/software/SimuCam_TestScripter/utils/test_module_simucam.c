...
bool bDdr2SingleMemoryZeroFill(alt_u8 ucMemId) {
	bool bSuccess = FALSE;
	...
	if (ucMemId <= 1) {
		...
		if (bMfilDmaTransfer(ucMemId, (alt_u32 *)0x00000000, DDR2_M1_MEMORY_SIZE)) {
			while ( ( TRUE == bMfilGetWrBusy() ) && ( FALSE == bMfilGetWrTimeoutErr() ) ) {
				usleep(100);
			}
			if ( FALSE == bMfilGetWrTimeoutErr() ) {
#if DEBUG_ON
				fprintf(fp, "  DDR2 Memory %u Cleared!\n", ucMemId + 1);
				bSuccess = TRUE;
#endif
			} else {
#if DEBUG_ON
				fprintf(fp, "  CRITICAL! DDR2 Memory %u Failure!\n", ucMemId + 1);
				return (bSuccess);
#endif
			}
		}
