...
begin
	...
	ftdi_umft601a_controller_ent_inst : entity work.ftdi_umft601a_controller_ent
		port map(
			...
			--			ftdi_module_stop_i            => s_config_write_registers.ftdi_module_control_reg.ftdi_module_stop,
			--			ftdi_module_start_i           => s_config_write_registers.ftdi_module_control_reg.ftdi_module_start,
			ftdi_module_clear_i           => '0',
			ftdi_module_stop_i            => '0',
			ftdi_module_start_i           => '1',
	...
	-- Loopback feature (for sw tests)
	p_ftdi_usb3_loopback : process(a_avs_clock, a_reset) is
		variable v_operation_delay : natural := 0;
	begin
		if (a_reset = '1') then
			s_umft601a_tx_dc_data_fifo_wrdata_data <= (others => '0');
			s_umft601a_tx_dc_data_fifo_wrdata_be   <= (others => '0');
			s_umft601a_tx_dc_data_fifo_wrreq       <= '0';
			s_umft601a_rx_dc_data_fifo_rdreq       <= '0';
			v_operation_delay                      := 0;
			...
			-- check if not in the middle of an operation delay
			if (v_operation_delay = 0) then
				-- not in the middle of an operation delay
				-- check if there is data available in the rx data fifo and space available in the tx data fifo
				if ((s_umft601a_rx_dc_data_fifo_rdempty = '0') and (s_umft601a_tx_dc_data_fifo_wrfull = '0')) then
					-- there is data available in the rx data fifo and space available in the tx data fifo
					-- write rx data to the tx data fifo
					s_umft601a_tx_dc_data_fifo_wrdata_data <= s_umft601a_rx_dc_data_fifo_rddata_data;
					s_umft601a_tx_dc_data_fifo_wrdata_be   <= s_umft601a_rx_dc_data_fifo_rddata_be;
					s_umft601a_tx_dc_data_fifo_wrreq       <= '1';
					-- read rx data
					s_umft601a_rx_dc_data_fifo_rdreq       <= '1';
					-- set operation delay for 3 clock cycles
					v_operation_delay                      := 3;
				end if;
			else
				-- middle of an operation delay
				-- decrement operation delay
				v_operation_delay := v_operation_delay - 1;
			end if;

		end if;
	end process p_ftdi_usb3_loopback;
